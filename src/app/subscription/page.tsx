...
export default function SubscriptionPage() {
    ...
    const filteredPlans = plans?.filter(plan => plan.billing_cycle === billingCycle || plan.plan_type === 'trial') || []

    return (
        <div className="space-y-10 pb-10">
            {/* Header */}
            <div className="text-center max-w-2xl mx-auto">
                <h1 className={`text-4xl font-medium mb-4 font-poppins tracking-tight ${isDark ? 'text-white' : 'text-gray-900'}`}>
            ...
            <div className="flex justify-center mb-8">
                <div className={`flex items-center p-1 rounded-full border ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-gray-100 border-gray-200'}`}>
                    <button
                    ...
                    >
                        Anual
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${isDark ? 'bg-green-900/40 text-green-400' : 'bg-green-100 text-green-700'}`}>15% OFF</span>
                    </button>
                </div>
            </div>
            ...
            <div className={`max-w-4xl mx-auto rounded-xl p-4 sm:p-6 border transition-all ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="flex-1">
                        <h3 className={`font-medium text-base sm:text-lg mb-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    ...
                    <a
                        ...
                        className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-3 rounded-lg font-medium text-sm transition-all shadow-md hover:shadow-lg whitespace-nowrap bg-green-600 hover:bg-green-700 text-white"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className={`rounded-2xl p-8 hover:shadow-lg transition-all duration-300 border ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                    <h3 className={`text-xl font-medium mb-6 flex items-center gap-3 font-poppins ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    ...
                    <div className="space-y-6">
                        {[
                            ...
                            <div key={item.title} className="flex items-start gap-4">
                                ...
                                <div>
                                    <p className={`font-medium text-lg ${isDark ? 'text-white' : 'text-gray-900'}`}>{item.title}</p>
                    ...
                    <div className="space-y-6">
                        {[
                            ...
                            <div key={i}>
                                ...
                                <h4 className={`font-medium text-lg mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>{item.q}</h4>
