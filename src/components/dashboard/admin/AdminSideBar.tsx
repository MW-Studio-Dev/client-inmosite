// components/admin/AdminSidebar.tsx - Versión simplificada para aislar el problema
'use client'
...
function SidebarContent({
  ...
  // Añadimos un estado para controlar si el componente está montado
  const [isMounted, setIsMounted] = React.useState(false)
  
  // Usamos useEffect para marcar el componente como montado
  React.useEffect(() => {
    setIsMounted(true)
    return () => {
      setIsMounted(false)
  ...
  // Si el componente no está montado, no renderizamos nada
  if (!isMounted) {
    return null
  }

  return (
    <div className={`flex grow flex-col overflow-y-auto shadow-sm transition-colors duration-300 ${
      isDark
...
function MenuItem({
  ...
  return (
    <li>
      {hasSubItems ? (
        // Para elementos con subitems, usamos un div con manejador de eventos
        <div
          onClick={handleToggle}
          className={`
            group flex items-center rounded-lg transition-all duration-200 relative w-full text-left cursor-pointer
        ...
        // Para elementos sin subitems, usamos el componente Link de Next.js
        <Link
          href={item.href}
          className={`
            group flex items-center rounded-lg transition-all duration-200 relative w-full text-left
            ${isCollapsed ? 'px-2 py-3 justify-center' : 'px-3 py-2.5'}
            ${isActive
              ? isDark
                ? 'bg-gray-700 text-white shadow-sm'
      ...
      {hasSubItems && !isCollapsed && (
        <ul className={`mt-1 ml-9 space-y-1 overflow-hidden transition-all duration-200 ${
            ...
            return (
              <li key={subItem.href}>
                <Link
                  href={subItem.href}
                  className={`
                    flex items-center w-full px-3 py-2 text-sm rounded-lg transition-all duration-200 text-left
                    ${isSubActive
                      ? isDark
